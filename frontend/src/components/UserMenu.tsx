    ...
    useEffect(() => {
        if (isOpen && menuRef.current && !isMobile) {
            ...
            if (rect.right - dropdownWidth < 8) {
                rightOffset = window.innerWidth - dropdownWidth - 8;
            }
            setDropdownStyle({
            ...
            <AnimatePresence>
                {isOpen && mounted && (
                    <>
                        ...
                        <motion.div
                            initial={isMobile ? { opacity: 0, scale: 0.9, y: -20, x: "-50%" } : { opacity: 0, scale: 0.95, y: -8 }}
                            animate={isMobile ? { opacity: 1, scale: 1, y: 0, x: "-50%" } : { opacity: 1, scale: 1, y: 0 }}
                            exit={isMobile ? { opacity: 0, scale: 0.9, y: -20, x: "-50%" } : { opacity: 0, scale: 0.95, y: -8 }}
                            ...
                            className={cn(
                                ...
                                isMobile
                                    ? "fixed top-[15%] left-1/2 w-[min(288px,calc(100vw-32px))] max-h-[70vh] rounded-[2rem]"
                                    : "fixed max-h-[80vh] rounded-xl"
                            )}
                        >
                            {/* Header Section */}
