...
export function Navbar() {
    ...
    if (pathname && (pathname.startsWith('/secure-admin-login') || pathname.startsWith('/admin') || pathname === '/maintenance' || pathname === '/banned')) {
        return null;
    }

    return (
        <>
            {/* Premium Top Progress Bar (Simulated for transitions) */}
            <motion.div
                key={pathname}
                initial={{ width: "0%", opacity: 1 }}
                animate={{ width: "100%", opacity: 0 }}
                transition={{ duration: 1.5, ease: "easeInOut" }}
                className="fixed top-0 left-0 h-[3px] bg-gradient-to-r from-primary via-blue-500 to-purple-600 z-[99999]"
            />
            {/* Announcements */}
                ...
                <div className="container flex h-16 items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="hidden md:flex items-center gap-4">
                            {user ? (
                                <>
                                    ...
                                    <Link href="/settings" className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted/50 border border-border/50 hover:border-primary/50 transition-all group">
                                        <div className="relative h-6 w-6 flex items-center justify-center">
                                            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                                <circle cx="18" cy="18" r="16" fill="none" className="stroke-muted/20" strokeWidth="4" />
                                                <circle 
                                                    cx="18" cy="18" r="16" fill="none" 
                                                    className={cn(user.profileCompletion === 100 ? "stroke-green-500" : "stroke-primary")}
                                                    strokeWidth="4" strokeDasharray={`${user.profileCompletion}, 100`} 
                                                    strokeLinecap="round"
                                                    style={{ transition: 'stroke-dasharray 1s ease-in-out' }}
                                                />
                                            </svg>
                                            <span className="absolute text-[8px] font-bold">{user.profileCompletion || 0}%</span>
                                        </div>
                                        <span className="text-[10px] font-bold text-muted-foreground group-hover:text-foreground uppercase tracking-wider">Setup</span>
                                    </Link>
                                    <NotificationsMenu />
                        ...
                        <div className="md:hidden flex items-center gap-2">
                            {user ? (
                                <>
                                    {/* Profile Completion - Mobile Mini */}
                                    <Link href="/settings" className="flex items-center gap-1 px-2 py-1 rounded-full bg-muted/30 border border-border/30">
                                        <div className="relative h-5 w-5 flex items-center justify-center">
                                            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                                                <circle cx="18" cy="18" r="16" fill="none" className="stroke-muted/10" strokeWidth="5" />
                                                <circle 
                                                    cx="18" cy="18" r="16" fill="none" 
                                                    className={cn(user.profileCompletion === 100 ? "stroke-green-500" : "stroke-primary")}
                                                    strokeWidth="5" strokeDasharray={`${user.profileCompletion}, 100`} 
                                                    strokeLinecap="round"
                                                />
                                            </svg>
                                            <span className="absolute text-[7px] font-bold">{user.profileCompletion || 0}%</span>
                                        </div>
                                    </Link>
                                    <NotificationsMenu />
                                    <UserMenu user={user} onLogout={handleLogout} />
                    ...
                    >
                        <div className="container py-2.5 px-4 flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <p className="text-[11px] sm:text-xs font-bold text-yellow-600 dark:text-yellow-500 uppercase tracking-wide">
                                    Profile Incomplete ({user.profileCompletion || 0}%) — Completing it is required for tournament registration.
                                </p>
                            </div>
                            <Link href="/settings">
                                <Button size="sm" variant="outline" className="h-8 bg-yellow-500/10 border-yellow-500/30 text-yellow-600 dark:text-yellow-500 text-[10px] font-black tracking-widest hover:bg-yellow-500 hover:text-white transition-all uppercase">
                                    Complete Now
                                </Button>
                            </Link>
                        </div>
