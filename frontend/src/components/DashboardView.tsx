                                    ...
                                    <div className="space-y-4">
                                        {data.upcomingTournaments.map((t: any) => (
                                            <Link key={t.id} href={`/tournaments/${t.id}`}>
                                                <motion.div
                                                    whileHover={{ x: 5 }}
                                                    className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-border rounded-2xl bg-muted/20 hover:bg-muted/50 transition-all cursor-pointer group gap-4"
                                                >
                                                    <div className="flex items-center gap-4 min-w-0">
                                                        <div className="bg-primary/10 p-3 rounded-xl border border-primary/20 group-hover:scale-110 transition-transform shrink-0">
                                                            <Trophy className="h-6 w-6 text-primary" />
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <h3 className="font-bold font-orbitron text-base sm:text-lg group-hover:text-primary transition-colors truncate">{t.title}</h3>
                                                    ...
                                                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 pt-3 sm:pt-0 border-t sm:border-t-0 border-border/40 sm:ml-4">
                                                        <Badge className="bg-primary hover:bg-primary text-white font-orbitron px-3 py-1 text-[10px] sm:text-xs whitespace-nowrap order-1 sm:order-none">{t.prizePool} Coins</Badge>
                                                        <span className="text-[9px] sm:text-[10px] uppercase font-bold text-muted-foreground tracking-widest whitespace-nowrap order-2 sm:order-none">Entry: {t.entryFeePerPerson} Coins</span>
                                                    </div>
                                                </motion.div>
                                            </Link>
                                        ))}
