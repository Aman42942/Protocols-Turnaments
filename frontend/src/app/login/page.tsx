...
function LoginContent() {
    ...
    // Check for URL errors (e.g. from OAuth redirects)
    React.useEffect(() => {
        const errorMsg = searchParams.get('message');
        if (errorMsg) {
            setError(decodeURIComponent(errorMsg));
            // Remove it from the URL gracefully without reloading
    ...
    const handleLogin = (data: any) => {
        ...
        if (data.user.role === 'ADMIN' || data.user.role === 'SUPERADMIN' || data.user.role === 'ULTIMATE_ADMIN') {
            router.push('/admin');
        } else {
    ...
    if (step === 'otp') {
        return (
            <div className="min-h-screen flex flex-col items-center justify-start py-8 md:py-20 p-4 bg-muted/30 overflow-y-auto">
            ...
            <Card className="w-full max-w-md shadow-lg">
                ...
                <CardContent>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Button variant="outline" type="button" disabled={loading} onClick={() => window.location.href = `${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000').replace(/\/$/, '')}/api/auth/google`}>
                            <svg className="mr-2 h-4 w-4" aria-hidden="true" focusable="false" data-prefix="fab" data-icon="google" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 488 512"><path fill="currentColor" d="M488 261.8C488 403.3 391.1 504 248 504 110.8 504 0 393.2 0 256S110.8 8 248 8c66.8 0 123 24.5 166.3 64.9l-67.5 64.9C258.5 52.6 94.3 116.6 94.3 256c0 86.5 69.1 156.6 153.7 156.6 98.2 0 135-70.4 140.8-106.9H248v-85.3h236.1c2.3 12.7 3.9 24.9 3.9 41.4z"></path></svg>
                            Google
                        </Button>
                        <Button variant="outline" type="button" disabled={loading} onClick={() => window.location.href = `${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000').replace(/\/$/, '')}/api/auth/facebook`}>
