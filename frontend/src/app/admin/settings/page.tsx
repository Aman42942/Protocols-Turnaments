...
export default function AdminSettingsPage() {
    ...
    const [superAdminLimit, setSuperAdminLimit] = useState('50000');
    ...
    const fetchWithdrawalLimits = async () => {
        try {
            const res = await api.get('/wallet/admin/withdrawal-limits');
            setAdminLimit(res.data.adminLimit.toString());
            setSuperAdminLimit(res.data.superAdminLimit.toString());
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Save withdrawal limits to backend
            await api.post('/wallet/admin/withdrawal-limits', {
                adminLimit: Number(adminLimit),
                superAdminLimit: Number(superAdminLimit)
            });
                ...
                <Card>
                    ...
                    <CardContent className="space-y-5">
                        ...
                        <div className="space-y-4 pt-4 border-t">
                            <h4 className="text-sm font-bold uppercase tracking-wider text-primary">Role-Based Payout Limits (INR Equivalent)</h4>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                    <label className="text-sm font-medium">Admin Limit</label>
                                    <Input 
                                        type="number" 
                                        value={adminLimit} 
                                        ...
                                        placeholder="10000"
                                    />
                                    <p className="text-[10px] text-muted-foreground">Max amount an ADMIN can approve.</p>
                                </div>
                                <div className="grid gap-2">
                                    <label className="text-sm font-medium">Superadmin Limit</label>
                                    <Input 
                                        type="number" 
                                        value={superAdminLimit} 
                                        onChange={(e) => setSuperAdminLimit(e.target.value)} 
                                        placeholder="50000"
                                    />
                                    <p className="text-[10px] text-muted-foreground">Max amount a SUPERADMIN can approve.</p>
                                </div>
                            </div>
                            <p className="text-[10px] bg-primary/5 p-2 rounded border border-primary/10 italic">
                                Note: Ultimate Admin (Owner) always has <strong>Unlimited</strong> approval authority.
                            </p>
                        </div>
