...
export class OrganizerService {
    ...
    async create(userId: string, data: {
        ...
        const exists = await this.prisma.organizer.findUnique({ where: { userId } });
        if (exists) throw new ConflictException('You already have an organizer account');

        const slugTaken = await this.prisma.organizer.findUnique({ where: { slug: data.slug } });
    ...
    async findByUserId(userId: string) {
        ...
        if (!organizer) throw new NotFoundException('You do not have an organizer account');
        return organizer;
    }
    ...
    async upsertTheme(userId: string, themeData: {
        primaryColor?: string;
        accentColor?: string;
        bgColor?: string;
        textColor?: string;
    ...
    async getDashboard(userId: string) {
        ...
        const [
            ...
            // Live tournaments
            this.prisma.tournament.count({ where: { organizerId: org.id, status: 'LIVE' } }),

            // Completed tournaments
            this.prisma.tournament.count({ where: { organizerId: org.id, status: 'COMPLETED' } }),
            ...
            this.prisma.tournament.findMany({
                ...
                select: {
                    ...
                    entryFeePerPerson: true, prizePool: true, collectedFees: true,
