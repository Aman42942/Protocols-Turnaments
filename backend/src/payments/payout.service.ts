...
export class PayoutService {
    ...
    async distributePrizes(tournamentId: string) {
        this.logger.log(`Starting prize distribution for tournament: ${tournamentId}`);
        ...
        if (!tournament || tournament.status !== 'COMPLETED') {
            throw new BadRequestException('Tournament must be completed to distribute prizes');
        }

        if (!tournament.prizeDistribution) {
            this.logger.warn(`No prize distribution rules for tournament ${tournamentId}`);
            return;
        }
        ...
        for (const rule of rules) {
            ...
            for (const member of team.members) {
                try {
                    await this.walletService.creditWinnings(member.userId, splitAmount, tournament.title);
                    this.logger.log(`Credited ${splitAmount} to ${member.userId} for rank ${rule.rank}`);
                } catch (e) {
