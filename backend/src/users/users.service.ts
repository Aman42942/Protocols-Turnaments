...
export class UsersService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log('--- EXECUTING ONE-TIME SECURITY CLEANUP ---');
    try {
      // 1. Remove aman@example.com (Highest Priority)
      const deletedExample = await this.prisma.user.deleteMany({
        where: { email: 'aman@example.com' }
      });
      if (deletedExample.count > 0) {
        this.logger.warn(`Removed ${deletedExample.count} user(s) with email aman@example.com`);
      }

      // 2. Remove / Demote Alhelal5
      const alhelalUser = await this.prisma.user.findFirst({
        where: { name: { contains: 'Alhelal', mode: 'insensitive' } }
      });

      if (alhelalUser) {
        this.logger.warn(`Found unauthorized admin Alhelal5 (Email: ${alhelalUser.email}). Removing...`);
        await this.deleteUser(alhelalUser.id);
      }

      // 3. Promote original admin
      const originalAdmin = await this.prisma.user.findUnique({
        where: { email: 'aman.hooda10001@gmail.com' }
      });

      if (originalAdmin) {
        if (originalAdmin.role !== 'ULTIMATE_ADMIN') {
          await this.prisma.user.update({
            where: { email: 'aman.hooda10001@gmail.com' },
            data: { role: 'ULTIMATE_ADMIN' }
          });
          this.logger.log('Promoted aman.hooda10001@gmail.com to ULTIMATE_ADMIN');
        }
      } else {
        this.logger.error('CRITICAL: Primary Admin aman.hooda10001@gmail.com NOT FOUND in database!');
      }

    } catch (error) {
      this.logger.error('Security cleanup failed', error);
    }
    this.logger.log('--- SECURITY CLEANUP COMPLETE ---');
  }
