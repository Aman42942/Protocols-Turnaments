...
export class WalletController {
  ...
  async getWithdrawalLimits() {
    return this.walletService.getWithdrawalLimits();
  }

  @Post('admin/withdrawal-limits')
  @UseGuards(RolesGuard)
  @Roles('ADMIN')
  async updateWithdrawalLimits(
    @Body() body: { adminLimit: number; superAdminLimit: number },
    @Request() req,
  ) {
    const result = await this.walletService.updateWithdrawalLimits(
      body.adminLimit,
      body.superAdminLimit,
    );
    await this.activityLogService.log(
      req.user.userId,
      'UPDATE_WITHDRAWAL_LIMITS',
      { adminLimit: body.adminLimit, superAdminLimit: body.superAdminLimit },
      undefined,
      req.ip,
    );
    return result;
