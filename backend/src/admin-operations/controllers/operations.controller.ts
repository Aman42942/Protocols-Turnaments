...
export class OperationsController {
  ...
  async overrideScore(
    ...
    @Body() body: { teamId: string; placement: number; kills: number; twoFactorToken?: string },
    @Request() req,
  ) {
    // Mock admin ID from request user or hardcoded for now if auth disabled
    const adminId = req.user?.id || 'admin-id-placeholder';
    return this.operationsService.overrideMatchScore(
      matchId,
      body.teamId,
      body.placement,
      body.kills,
  ...
  async triggerPayout(@Param('id') id: string, @Body('twoFactorToken') token: string, @Request() req) {
    const adminId = req.user?.id || 'admin-id-placeholder';
    return this.operationsService.triggerManualPayout(id, adminId, token);
  }
  ...
  async approveWithdrawal(
    ...
    return this.operationsService.approveWithdrawal(id, admin, token);
  }
}
