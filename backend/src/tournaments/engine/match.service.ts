...
export class MatchService {
    ...
    async submitMatchResults(
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            for (const res of results) {
                ...
                await tx.matchParticipation.upsert({
                    ...
                    create: { matchId, teamId: res.teamId, placement: res.placement, kills: res.kills, score },
                ...
                scoreUpdates.push({ teamId: res.teamId, newTotalScore: newTotalPoints });
            }
        });

        // Step 2: Update Redis sorted set (outside transaction — non-critical)
        await Promise.all(
            scoreUpdates.map(({ teamId, newTotalScore }) =>
        ...
        await this.leaderboardGateway.broadcastLeaderboardUpdate(match.tournamentId);
