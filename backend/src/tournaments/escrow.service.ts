...
import { FinancialService } from '../admin-operations/services/financial.service';
...
export class EscrowService {
  ...
  async lockPool(tournamentId: string, adminId: string) {
    ...
    await this.financial.trackRevenue(
      platformFee,
      'PLATFORM_FEE',
      tournamentId,
      `Platform Fee from tournament: ${tournamentId}`,
    );
  ...
  async distributePool(tournamentId: string, adminId: string) {
    ...
    // Track TDS Revenue
    const totalTds = tdsLogs.reduce((s, l) => s + l.amount, 0);
    if (totalTds > 0) {
      await this.financial.trackRevenue(
        totalTds,
        'TDS',
        tournamentId,
        `TDS Collected from tournament: ${tournamentId}`,
      );
    }
