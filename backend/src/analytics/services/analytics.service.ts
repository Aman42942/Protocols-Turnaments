...
export class AnalyticsService {
    ...
    async getPlayerStats(playerId: string) {
        ...
        const user = await this.prisma.user.findUnique({
            where: { id: playerId },
            select: { id: true, name: true, avatar: true, riotId: true, pubgId: true }
        });
        ...
        if (teamIds.length === 0) {
            return { user, matchesPlayed: 0, wins: 0, winRate: 0, kdRatio: 0, avgPlacement: 0 };
        }
        ...
        const totalPlacement = participations.reduce((sum, p) => sum + (p.placement || 20), 0); // Default 20 if null
    ...
    async getTeamStats(teamId: string) {
        ...
        if (matchesPlayed === 0) {
            return { team, matchesPlayed: 0, consistencyScore: 0, top3Rate: 0, recentResults: [] };
        }
        ...
        const variance = placements.reduce((a, b) => a + Math.pow(b - avgPlacement, 2), 0) / matchesPlayed;
    ...
    async handleDailyStats() {
        ...
        const matchesPlayed = await this.prisma.match.count({
            where: { startTime: { gte: yesterday, lte: endOfYesterday }, status: 'COMPLETED' }
        });
        ...
        this.logger.log(`Daily Stats computed for ${yesterday.toISOString().split('T')[0]}`);
    }
}
