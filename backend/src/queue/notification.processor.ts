    ...
    async process(job: Job): Promise<any> {
        switch (job.name) {
            case NOTIF_JOB_TOURNAMENT_START: {
                const { tournamentId, tournamentTitle, startDate } = job.data as TournamentStartJobData;
                this.logger.log(`[${job.id}] Notifying participants of ${tournamentTitle} starting`);
                ...
                for (const p of participants) {
                    ...
                    ).catch((e) => this.logger.error(`Email failed for ${p.user.email}: ${e.message}`));
                }
                return { sent: participants.length };
            }

            case NOTIF_JOB_PRIZE_SENT: {
                const { userId, amount, tournamentTitle, place } = job.data as PrizeSentJobData;
