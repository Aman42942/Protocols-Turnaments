    ...
    async process(job: Job<PayoutJobData>): Promise<any> {
        ...
        switch (job.name) {
            case PAYOUT_JOB_DISTRIBUTE: {
                this.logger.log(`[${job.id}] Distributing prizes for tournament ${tournamentId}`);
                const result = await this.escrow.distributePool(tournamentId, adminId);
                this.logger.log(`[${job.id}] Distribution complete: ${JSON.stringify(result)}`);
                return result;
            }
            ...
            case PAYOUT_JOB_REFUND: {
                ...
                this.logger.log(`[${job.id}] Refund complete: ${JSON.stringify(result)}`);
                return result;
            }
