...
export class CmsService {
    ...
    async getFullConfig() {
        ...
        try {
            ...
            const [themeRes, contentRes, layoutRes, featuresRes, slidesRes, presetsRes] = results;

            const theme = themeRes.status === 'fulfilled' ? themeRes.value : (this.configCache?.theme || this.DEFAULT_BLUE_THEME);
            const content = contentRes.status === 'fulfilled' ? contentRes.value : (this.configCache?.content || {});
            const layout = layoutRes.status === 'fulfilled' ? layoutRes.value : (this.configCache?.layout || []);
            const features = featuresRes.status === 'fulfilled' ? featuresRes.value : (this.configCache?.features || []);
            const slides = slidesRes.status === 'fulfilled' ? slidesRes.value : (this.configCache?.slides || []);
            const presets = presetsRes.status === 'fulfilled' ? presetsRes.value : (this.configCache?.presets || []);

            this.configCache = { theme, content, layout, features, slides, presets };
            this.lastCacheTime = now;
            return this.configCache;
        } catch (error) {
            this.logger.error('Critical failure in getFullConfig', error);
            // If even Promise.allSettled fails (rare), return cache even if stale, or empty defaults
            return this.configCache || { 
                theme: this.DEFAULT_BLUE_THEME, 
                content: {}, 
                layout: [], 
                features: [], 
                slides: [], 
